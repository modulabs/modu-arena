...
```json
{
  "hooks": {
    "PostToolUse": [{
      "type": "command",
      "command": "uv run \"$CLAUDE_PROJECT_DIR//.claude/hooks/modu/post_tool__linter.py\""
    }]
  }
}
```
...
uv run .claude/hooks/modu/session_start__show_project_info.py

# ❌ WRONG: Direct execution (may fail on Windows)
./.claude/hooks/modu/session_start__show_project_info.py
```
