...
- Implements Step 4 of Modu's 4-step workflow (Report and Commit)
...
- .modu/config/config.yaml (git strategy, language settings)
- .modu/config/sections/git-strategy.yaml (auto_branch, branch creation policy)
- .modu/config/sections/language.yaml (git_commit_messages setting)
- .modu-arena/specs/ directory listing (SPEC documents for sync)
- .modu/project/ directory listing (project documents for conditional update)
...
- Create backup directory: .modu-backups/sync-{timestamp}/
...
- Generate sync report: .modu/reports/sync-report-{timestamp}.md
...
Purpose: Update .modu/project/ documents when significant structural changes are detected.
...
Skip condition: If .modu/project/ directory does not exist or contains no files, skip this step entirely.
