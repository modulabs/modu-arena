# Modu Skill Reference

Common patterns, flag reference, legacy command mapping, and configuration files used across all Modu workflows.
...
- Modu Workflow: Parallel exploration phase, then sequential SPEC generation and DDD implementation
...
- modu --resume SPEC-XXX: Resume full autonomous workflow from last phase
...
- .modu/config/config.yaml: Main configuration file (merged from section files)
- .modu/config/sections/language.yaml: Language settings (conversation_language, agent_prompt_language, code_comments)
- .modu/config/sections/user.yaml: User identification (name)
- .modu/config/sections/quality.yaml: TRUST 5 framework settings, LSP quality gates, test coverage targets
- .modu/config/sections/system.yaml: System metadata (modu.version)

### Project Documentation

- .modu/project/product.md: Product overview, features, user value
- .modu/project/structure.md: Project architecture and directory organization
- .modu/project/tech.md: Technology stack, dependencies, technical decisions
...
- .modu/cache/release-snapshots/latest.json: Release state snapshot for recovery

### Version Files (5 files synchronized during release)

- pyproject.toml: Authoritative version source
- src/modu_adk/version.py: Runtime fallback version
- .modu/config/config.yaml: Config display version
- .modu/config/sections/system.yaml: System metadata version
- src/modu_adk/templates/.modu/config/sections/system.yaml: Template version for distribution
...
- `<modu>DONE</modu>`: Single task or phase completed
- `<modu>COMPLETE</modu>`: Full workflow completed (all phases finished)
