...
| `ralph.ast_grep.config_path`                        | string  | `.claude/skills/modu-tool-ast-grep/rules/sgconfig.yml` | Path to AST-grep configuration                            |
...
| ------------------------------ | ------- | -------------------------------------- | -------------------------------- |
| `MODU_DISABLE_LSP_DIAGNOSTIC`  | boolean | Disable LSP diagnostic hook            | `MODU_DISABLE_LSP_DIAGNOSTIC=1`  |
| `MODU_DISABLE_LOOP_CONTROLLER` | boolean | Disable loop controller hook           | `MODU_DISABLE_LOOP_CONTROLLER=1` |
| `MODU_LOOP_ACTIVE`             | boolean | Loop active flag (set by commands)     | `MODU_LOOP_ACTIVE=1`             |
| `MODU_LOOP_ITERATION`          | integer | Current iteration number               | `MODU_LOOP_ITERATION=3`          |
...
export MODU_DISABLE_LSP_DIAGNOSTIC=1
claude -p "Make changes"

# Disable loop controller
export MODU_DISABLE_LOOP_CONTROLLER=1
...
export MODU_LOOP_ACTIVE=1
export MODU_LOOP_ITERATION=0
...
```python
ModuLSPClient(project_root: str | Path)
```
...
client = ModuLSPClient(project_root="/path/to/project")
```
...
```python
from modu_adk.lsp.models import Position
...
```python
from modu_adk.lsp.models import Range, Position
...
```python
from modu_adk.lsp.models import Diagnostic, DiagnosticSeverity, Range, Position
...
```python
from modu_adk.lsp.models import Location, Range, Position
...
```python
from modu_adk.lsp.models import TextEdit, Range, Position
...
```python
from modu_adk.lsp.models import WorkspaceEdit, TextEdit, Range, Position
...
```python
from modu_adk.lsp.models import HoverInfo, Range, Position
...
### Loop State File (.modu_loop_state.json)

Location: `.modu/cache/.modu_loop_state.json`
...
Location: `.claude/skills/modu-tool-ast-grep/rules/sgconfig.yml`
...
1. Check if Ralph is enabled:
   ...
   ```yaml
   # .modu/config/sections/ralph.yaml
   ralph:
     enabled: true
   ```
...
3. Check environment variable:
   ...
   ```bash
   unset MODU_DISABLE_LOOP_CONTROLLER
   ```

4. Verify state file is writable:
   ```bash
   mkdir -p .modu/cache
   chmod 755 .modu/cache
   ```
...
4. Check environment variable:
   ...
   ```bash
   unset MODU_DISABLE_LSP_DIAGNOSTIC
   ```
...
3. Delete state file:
   ```bash
   rm .modu/cache/.modu_loop_state.json
   ```
...
3. Check configuration file exists:
   ...
   ```bash
   ls .claude/skills/modu-tool-ast-grep/rules/sgconfig.yml
   ```
    ...
    steps:
      ...
      - name: Commit fixes
        ...
        run: |
          ...
          git config user.email "ralph@modu-adk.dev"
          git add .
