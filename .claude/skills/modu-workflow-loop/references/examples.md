...
```json
// .modu/cache/.modu_loop_state.json
{
  "active": true,
  "iteration": 1,
  "max_iterations": 10,
...
ralph:
  ...
  ast_grep:
    ...
    quality_scan: true
    custom_rules:
      - .modu/ast-grep/security/**/*.yml
      - .modu/ast-grep/quality/**/*.yml
...
ralph:
  ...
  ast_grep:
    enabled: true
    rules:
      - .modu/ast-grep/react/**/*.yml
      - .modu/ast-grep/typescript/**/*.yml
```
...
def load_loop_state() -> dict:
    state_file = Path(".modu/cache/.modu_loop_state.json")
...
```yaml
# .modu/ast-grep/security/sql-injection.yml
...
```yaml
# .modu/ast-grep/security/xss-prevention.yml
id: xss-react
language: typescript
rule:
...
```yaml
# .modu/ast-grep/security/hardcoded-secrets.yml
      ...
      - name: Run Ralph Loop
        run: |
          claude -p "/modu:loop --max-iterations 5" \
            ...
            --output-format json
        env:
          MODU_LOOP_ACTIVE: "true"
          CLAUDE_API_KEY: ${{ secrets.CLAUDE_API_KEY }}

      - name: Check Results
        run: |
          python .modu/scripts/check_ralph_results.py
```
    ...
    steps:
      ...
      - name: Upload Results
        ...
        with:
          ...
          path: .modu/reports/${{ matrix.check }}
```
...
# Run LSP diagnostics
modu lsp diagnose --changed-files
...
modu ast-grep scan --security --changed-files
...
```bash
# View LSP logs
cat .modu/logs/lsp_diagnostic.log

# Test LSP connection
modu lsp test-connection python

# View diagnostics directly
modu lsp diagnose src/auth.py
```
...
```bash
# View current state
cat .modu/cache/.modu_loop_state.json

# Reset loop state
rm .modu/cache/.modu_loop_state.json

# View loop logs
cat .modu/logs/loop_controller.log
```
...
Check:
  ...
  - MODU_DISABLE_LOOP_CONTROLLER not set
  - State file writable
