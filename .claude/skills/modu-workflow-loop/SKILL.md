...
The Stop hook is triggered after each Claude response. The hook reads the loop state file located at .modu/cache/.modu_loop_state.json. This state contains active status (true or false), current iteration number, max_iterations limit, last_error_count from previous iteration, and completion_reason when finished.
...
Initialize the ModuLSPClient from modu_adk.lsp.client with the project_root parameter set to the project directory path.
...
For GitHub Actions integration, create a workflow step that runs Claude with the /modu:loop command and max-iterations flag. Set the MODU_LOOP_ACTIVE environment variable to true to enable loop mode.
...
Check that ralph.enabled is set to true in configuration. Verify MODU_DISABLE_LOOP_CONTROLLER environment variable is not set. Ensure the state file location is writable.

### LSP Diagnostics Missing

Check LSP server configuration in .lsp.json file. Verify the language server is installed for your language. Check that MODU_DISABLE_LSP_DIAGNOSTIC environment variable is not set.

### Loop Stuck

Review the max_iterations setting to ensure it allows sufficient iterations. Review completion conditions to verify they are achievable. Send any message to interrupt the loop, or delete the state file (.modu/cache/.modu_loop_state.json) to reset.

## Works Well With

Skills:

- modu-foundation-quality: TRUST 5 validation
- modu-tool-ast-grep: Security scanning patterns
- modu-workflow-testing: DDD integration
- modu-lang-python: Python-specific patterns
- modu-lang-typescript: TypeScript patterns
...
MODU_DISABLE_LSP_DIAGNOSTIC disables the LSP hook when set.

MODU_DISABLE_LOOP_CONTROLLER disables the loop hook when set.

MODU_LOOP_ACTIVE indicates whether the loop is currently active.

MODU_LOOP_ITERATION contains the current iteration number.

CLAUDE_PROJECT_DIR contains the project root path.

### File Locations

Configuration is stored at .modu/config/sections/ralph.yaml.

Loop state is stored at .modu/cache/.modu_loop_state.json.
...
Skill Name: modu-workflow-loop (formerly modu-ralph)
