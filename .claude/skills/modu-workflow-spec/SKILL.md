---
name: modu-workflow-spec
description: >
  SPEC workflow orchestration with EARS format requirements, acceptance criteria,
  and Plan-Run-Sync integration for Modu-ADK development methodology.
  ...
  Do NOT use for implementation (use modu-workflow-ddd instead)
  or documentation generation (use modu-workflow-project instead).
license: Apache-2.0
...
metadata:
  ...
  updated: "2026-01-08"
  modularized: "true"
  tags: "workflow, spec, ears, requirements, modu-adk, planning"
  author: "Modu-ADK Team"
  context: "fork"
  agent: "Plan"
...
Constitution defines the project DNA that all SPECs must respect. Before creating any SPEC, verify alignment with project constitution defined in `.modu/project/tech.md`.
...
- modu-workflow-testing skill reference for test patterns
...
- .modu/memory/: Session state files (last-session-state.json)
- .modu/docs/: Generated documentation (api-documentation.md)
...
- modu-foundation-core: SPEC-First DDD methodology and TRUST 5 framework
- modu-workflow-testing: DDD implementation and test automation
- modu-workflow-project: Project initialization and configuration
- modu-workflow-worktree: Git Worktree management for parallel development
...
- Session memory in .modu/memory/ for cross-session context
...
| --------------------- | ----------------------------- | ----------------------------------------- |
| Security Audit        | Analyzes existing code        | `.modu/reports/security-audit-{DATE}/`    |
| Performance Report    | Documents current metrics     | `.modu/reports/performance-{DATE}/`       |
| Dependency Analysis   | Reviews existing dependencies | `.modu/reports/dependency-review-{DATE}/` |
| Architecture Overview | Documents current state       | `.modu/docs/architecture.md`              |
| API Reference         | Documents existing APIs       | `.modu/docs/api-reference.md`             |
| Meeting Notes         | Records decisions made        | `.modu/reports/meeting-{DATE}/`           |
| Retrospective         | Analyzes past work            | `.modu/reports/retro-{DATE}/`             |

### Exclusion Rules

[HARD] Report vs SPEC Distinction:

Reports analyze what EXISTS → `.modu/reports/`
SPECs define what will be BUILT → `.modu-arena/specs/`

[HARD] Documentation vs SPEC Distinction:

Documentation explains HOW TO USE → `.modu/docs/`
...
2. Create reports directory:
   ```bash
   mkdir -p .modu/reports/security-audit-2025-01/
   ```
3. Move content:
   ```bash
   mv .modu-arena/specs/SPEC-SECURITY-AUDIT/* .modu/reports/security-audit-2025-01/
