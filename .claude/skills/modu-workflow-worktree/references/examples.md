# Modu Worktree Examples

Purpose: Real-world usage examples and patterns for modu-worktree skill integration with Modu-ADK workflow.
...
/modu:1-plan "User Authentication System" --worktree

# Output:
# SPEC created: SPEC-AUTH-001
# Worktree created: .modu/worktrees/Modu-ADK/SPEC-AUTH-001
#
# Next steps:
# 1. Switch to worktree: modu-worktree switch SPEC-AUTH-001
# 2. Or use shell eval: eval $(modu-worktree go SPEC-AUTH-001)
# 3. Start development: /modu:2-run SPEC-AUTH-001

# Development Phase - Implement in isolated environment
eval $(modu-worktree go SPEC-AUTH-001)
/modu:2-run SPEC-AUTH-001

# Sync Phase - Synchronize and integrate
modu-worktree sync SPEC-AUTH-001
/modu:3-sync SPEC-AUTH-001

# Cleanup Phase - Remove completed worktree
modu-worktree remove SPEC-AUTH-001
```
...
modu-worktree new SPEC-AUTH-001 "User Authentication"
modu-worktree new SPEC-PAY-001 "Payment Processing"
modu-worktree new SPEC-DASH-001 "Dashboard Analytics"

# Switch between worktrees
modu-worktree switch SPEC-AUTH-001
# Work on authentication...

modu-worktree switch SPEC-PAY-001
# Work on payment system...

modu-worktree switch SPEC-DASH-001
# Work on dashboard...

# Check status of all worktrees
modu-worktree status --all

# Sync all worktrees
modu-worktree sync --all

# Clean up completed worktrees
modu-worktree clean --merged-only
```
...
```bash
# /modu:2-run automatically detects worktree environment
/modu:2-run SPEC-AUTH-001
...
modu-worktree status SPEC-AUTH-001
modu-worktree sync SPEC-AUTH-001 --include "src/"
modu-worktree config get worktree_root
```
...
/modu:1-plan "$SPEC_DESCRIPTION" --worktree --spec-id "$SPEC_ID"

# Check if worktree was created successfully
if modu-worktree list --format json | jq -r ".worktrees[\"$SPEC_ID\"]" > /dev/null; then
 echo " Worktree $SPEC_ID created successfully"

 # Phase 2: Develop
 echo " Phase 2: Switching to worktree for development..."
 cd $(modu-worktree go "$SPEC_ID")
...
WORKTREE_ROOT=".modu/worktrees/$PROJECT_NAME"
...
modu-worktree config set worktree_root "$WORKTREE_ROOT"
modu-worktree config set registry_type team

echo " Team worktree configuration completed"
echo "Team members can now join with: modu-worktree join --team $TEAM_NAME"
```
...
DEBUG=true""",
 ...
 # Save template
 template_path = Path.home() / ".modu-worktree/templates" / "fullstack.json"
...
# modu-worktree new SPEC-FULL-001 "Fullstack Application" --template fullstack
```
...
class WorktreeAutomation:
 ...
 "modu-worktree", "new", spec_id, description,
 "--template", template
 ...
 result = subprocess.run([
 "modu-worktree", "list", "--status", "active", "--format", "json"
 ], capture_output=True, text=True, check=True)
 ...
 "modu-worktree", "status", "--all", "--format", "json"
 ], capture_output=True, text=True, check=True)
 ...
 'action': 'modu-worktree clean --stale --days 30'
 })
 ...
 'action': 'modu-worktree optimize --analyze'
 })
...
modu-worktree new SPEC-DEBUG-001 "Debug Test" --verbose

# Problem 2: Worktree sync conflicts
echo " Resolving sync conflicts..."

# Check sync status
modu-worktree status SPEC-CONFLICT-001

# Interactive conflict resolution
modu-worktree sync SPEC-CONFLICT-001 --interactive

# Force sync (if appropriate)
modu-worktree sync SPEC-CONFLICT-001 --force

# Problem 3: Worktree registry corruption
echo " Repairing worktree registry..."

# Backup current registry
cp .modu/worktrees/PROJECT/.modu-worktree-registry.json .modu/worktrees/PROJECT/.modu-worktree-registry.json.backup

# Rebuild registry from worktree directories
modu-worktree config set registry_rebuild true
modu-worktree list --rebuild-registry

# Problem 4: Permission issues
echo " Fixing permission issues..."

# Check worktree permissions
ls -la .modu/worktrees/PROJECT/

# Fix permissions
chmod -R 755 .modu/worktrees/PROJECT/
```
...
Version: 1.0.0
Last Updated: 2025-11-29
Examples: Real-world usage patterns for modu-worktree integration
