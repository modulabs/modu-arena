# Modu Worktree Reference

Purpose: External resources, documentation, and additional learning materials for modu-worktree skill.
...
- Click Framework: [https://click.palletsprojects.com/](https://click.palletsprojects.com/)
 - Command-line interface framework used by modu-worktree
...
- Rich Library: [https://rich.readthedocs.io/](https://rich.readthedocs.io/)
 - Terminal output formatting used by modu-worktree
...
- modu-foundation-core: Foundation principles and delegation patterns
 - Reference: [modu-foundation-core modules](modu-foundation-core/modules/)
...
- modu-workflow-project: Project management and configuration
 - Reference: [modu-workflow-project modules](modu-workflow-project/modules/)
 - Integration: Project setup with worktree support

- modu-foundation-claude: Claude Code execution patterns
 - Reference: [modu-foundation-claude modules](modu-foundation-claude/modules/)
...
1. Shallow Worktrees: For fast prototyping and testing
 ```bash
 modu-worktree new SPEC-PROTO-001 "Prototype" --shallow --depth 1
 ```

2. Selective Synchronization: Sync only essential files
 ```bash
 modu-worktree sync SPEC-001 --include "src/" --exclude "node_modules/"
 ```

3. Background Operations: Non-blocking worktree operations
 ```bash
 modu-worktree sync --all --background
 ```
...
jobs:
 ...
 - name: Setup modu-worktree
 run: |
 uv tool install modu-adk
 echo "Setting up worktree environment..."

 - name: Test Worktree Operations
 run: |
 # Test worktree creation
 modu-worktree new test-spec "Test Worktree"

 # Test worktree synchronization
 modu-worktree sync test-spec

 # Test worktree cleanup
 modu-worktree remove test-spec
```
...
pipeline {
 ...
 sh '''
 uv tool install modu-adk
 modu-worktree config set worktree_root $WORKSPACE/worktrees
 '''
 }
 }
 ...
 steps {
 sh '''
 modu-worktree new SPEC-AUTH-001 "Authentication Worktree"
 cd $(modu-worktree go SPEC-AUTH-001)
 npm test
 '''
 }
 }
 ...
 steps {
 sh '''
 modu-worktree new SPEC-PAY-001 "Payment Worktree"
 cd $(modu-worktree go SPEC-PAY-001)
 npm test
 '''
 }
 }
...
- Integration with Modu-ADK workflow commands
...
git clone https://github.com/your-org/modu-adk.git
...
Version: 1.0.0
Last Updated: 2025-11-29
Reference: External resources and additional learning materials for modu-worktree
