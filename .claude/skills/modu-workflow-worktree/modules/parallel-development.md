...
To set up parallel development, create worktrees for each SPEC using modu-worktree new with the SPEC ID and description. For parallel execution, navigate to each worktree using modu-worktree go and run /modu:2-run in separate terminals or background processes. For integration, use modu-worktree sync --all to synchronize all worktrees, then clean up merged worktrees with modu-worktree clean --merged-only.
...
Worktree Root ({repo}/.modu/worktrees/{project-name}/):
