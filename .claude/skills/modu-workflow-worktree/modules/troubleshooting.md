...
- Integration Issues: Modu workflow integration and command coordination

Quick Diagnostics:
- Check worktree status: modu-worktree status --all
- Verify registry integrity: modu-worktree registry validate
...
Prevention:
- Always use modu-worktree remove instead of manual directory deletion
...
Symptoms:
- modu-worktree commands fail with JSON parsing errors
...
- Validate the rebuilt registry with modu-worktree status --all

### Orphaned Registry Entries

Symptoms:
- Registry lists worktrees that no longer exist on disk
- modu-worktree status shows worktrees as missing
...
Resolution Steps:
1. Run modu-worktree status --all to identify orphaned entries
2. For each orphaned entry, confirm the directory truly does not exist
3. Remove orphaned entries from registry using modu-worktree registry prune
...
- Use modu-worktree commands for standard operations
...
### Modu Command Coordination Failures

Symptoms:
- /modu:1-plan does not create expected worktree
- /modu:2-run cannot find worktree for SPEC
- Workflow commands operate on wrong worktree

Root Causes:
- SPEC ID not matching worktree ID format
- Worktree was created manually without Modu integration
- Configuration mismatch between Modu and worktree settings
- Working directory context not in expected location

Resolution Steps:
1. Verify SPEC ID format matches expected worktree naming convention
2. Ensure worktree was created through Modu workflow or properly registered
3. Check modu configuration for worktree integration settings
...
Status Commands:
- modu-worktree status --all: Shows all worktrees with sync status
- git worktree list: Native Git worktree listing
- modu-worktree status SPEC-ID --detailed: Detailed status for specific worktree

Registry Commands:
- modu-worktree registry validate: Checks registry integrity
- modu-worktree registry prune: Removes orphaned entries
- modu-worktree registry export: Exports registry for backup
...
Cleanup Commands:
- modu-worktree clean --dry-run: Preview cleanup without changes
- modu-worktree clean --merged-only: Clean only merged worktrees
- git worktree prune: Remove stale Git worktree metadata

Recovery Commands:
- modu-worktree registry rebuild: Reconstruct registry from directories
- modu-worktree remove SPEC-ID --keep-branch: Remove worktree, preserve branch
...
Weekly Tasks:
- Run modu-worktree status --all to check for issues
...
Creation:
- Always use modu-worktree new instead of manual Git commands
...
Removal:
- Use modu-worktree remove instead of manual deletion
