...
Quick Start:
1. Create worktree: modu-worktree new SPEC-001 "User Authentication"
2. Switch to worktree: modu-worktree switch SPEC-001
3. Or use shell eval: eval $(modu-worktree go SPEC-001)

---

## Creation Commands

### modu-worktree new - Create Worktree

Create a new isolated Git worktree for SPEC development.

Syntax: modu-worktree new <spec-id> [description] [options]
...
Examples:
- Basic creation: modu-worktree new SPEC-001 "User Auth System"
- Custom branch: modu-worktree new SPEC-002 "Payment" --branch feature/payment-gateway
- From develop: modu-worktree new SPEC-003 "API Refactor" --base develop
- With template: modu-worktree new SPEC-004 "Frontend" --template frontend
- Fast creation: modu-worktree new SPEC-005 "Bug Fixes" --shallow --depth 1
...
### modu-worktree list - List Worktrees

Display all registered worktrees with their status and metadata.

Syntax: modu-worktree list [options]
...
Examples:
- Table format: modu-worktree list
- JSON output: modu-worktree list --format json
- Active only: modu-worktree list --status active
- Sort by date: modu-worktree list --sort created
- Detailed: modu-worktree list --verbose

### modu-worktree switch - Switch to Worktree

Change current working directory to the specified worktree.

Syntax: modu-worktree switch <spec-id> [options]
...
Examples:
- Basic switch: modu-worktree switch SPEC-001
- With sync: modu-worktree switch SPEC-002 --auto-sync
- Force switch: modu-worktree switch SPEC-003 --force

### modu-worktree go - Get Worktree Path

Output the cd command for shell integration.

Syntax: modu-worktree go <spec-id> [options]
...
- eval pattern (recommended): eval $(modu-worktree go SPEC-001)
- source pattern: modu-worktree go SPEC-001 | source
- manual cd: cd $(modu-worktree go SPEC-001 --absolute)

---

## Management Commands

### modu-worktree sync - Synchronize Worktree

Synchronize worktree with its base branch.

Syntax: modu-worktree sync <spec-id> [options]
...
Examples:
- Sync specific: modu-worktree sync SPEC-001
- Sync all: modu-worktree sync --all
- Interactive: modu-worktree sync SPEC-001 --interactive
- Preview: modu-worktree sync SPEC-001 --dry-run
- Include pattern: modu-worktree sync SPEC-001 --include "src/"
- Exclude pattern: modu-worktree sync SPEC-001 --exclude "node_modules/"
...
### modu-worktree remove - Remove Worktree

Remove a worktree and clean up its registration.

Syntax: modu-worktree remove <spec-id> [options]
...
Examples:
- Interactive: modu-worktree remove SPEC-001
- Force: modu-worktree remove SPEC-001 --force
- Keep branch: modu-worktree remove SPEC-001 --keep-branch
- With backup: modu-worktree remove SPEC-001 --backup
- Preview: modu-worktree remove SPEC-001 --dry-run

### modu-worktree clean - Clean Up Worktrees

Remove worktrees for merged branches or stale worktrees.

Syntax: modu-worktree clean [options]
...
Examples:
- Merged only: modu-worktree clean --merged-only
- Stale (30 days): modu-worktree clean --stale
- Custom threshold: modu-worktree clean --stale --days 14
- Interactive: modu-worktree clean --interactive
- Preview: modu-worktree clean --dry-run
- Force: modu-worktree clean --force

---

## Status and Configuration

### modu-worktree status - Show Worktree Status

Display detailed status information about worktrees.

Syntax: modu-worktree status [spec-id] [options]
...
Examples:
- Current worktree: modu-worktree status
- Specific worktree: modu-worktree status SPEC-001
- All with sync check: modu-worktree status --all --sync-check
- Detailed Git status: modu-worktree status SPEC-001 --detailed
- JSON output: modu-worktree status --all --format json
...
### modu-worktree config - Configuration Management

Manage modu-worktree configuration settings.

Syntax: modu-worktree config <action> [key] [value]
...
Examples:
- List all: modu-worktree config list
- Get value: modu-worktree config get worktree_root
- Set value: modu-worktree config set auto_sync true
- Reset: modu-worktree config reset worktree_root
- Edit: modu-worktree config edit
...
- modu-worktree clean --merged-only --force
