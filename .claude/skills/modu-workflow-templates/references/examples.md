...
**Input**:
```python
from modu_workflow_templates import load_template
...
---
Generated by: Modu-ADK Feedback System
...
**Scenario**: Modu-ADK 업데이트 후 사용자 커스터마이징을 보존하며 템플릿 병합
...
```json
{
  ...
  "conflicts_resolved": [
    {
      "file": ".modu/config/sections/quality.yaml",
      "resolution": "Merged both changes",
      "details": "User's custom thresholds + new quality metrics"
    }
  ],
  "manual_review_required": [],
  "restoration_point": ".modu/backups/pre-merge-2025-12-06/"
}
```
...
```json
// .modu/config/template-history.json
{
...
```bash
# 잘못된 예시 - 템플릿 직접 수정
vim .claude/skills/modu-workflow-templates/modules/code-templates.md
# → Modu-ADK 업데이트 시 충돌
# → 팀원 간 불일치
```
...
# → .modu/config/template-overrides.json에 저장
# → 템플릿 업데이트와 독립적
```
