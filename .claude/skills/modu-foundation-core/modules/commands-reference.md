# Commands Reference - Modu-ADK Core Commands

Purpose: Complete reference for Modu-ADK's 6 core commands used in SPEC-First DDD workflow.
...
Modu-ADK provides 6 core commands for SPEC-First DDD execution:
...
What It Does:
1. Creates `.modu/` directory structure
2. Generates `config.json` with default settings
3. Initializes Git repository (if `--with-git` flag provided)
4. Sets up Modu-ADK workflows

Output:
- `.modu/` directory
- `.modu/config/config.yaml`
- `.modu/memory/` (empty, ready for session state)
- `.modu/logs/` (empty, ready for logging)
...
Alfred: Project initialized successfully.
 - .modu/config/config.yaml created
...
Alfred: Documentation synchronized for SPEC-001.
 ...
 Generated:
 - API documentation: .modu/docs/SPEC-001/api.yaml
 - Architecture diagram: .modu/docs/SPEC-001/architecture.md
 - Completion report: .modu/docs/SPEC-001/report.md
```
...
3. Reports to Modu-ADK development team
4. Proposes error recovery strategies

Use Cases:
- Errors: When errors occur during any workflow phase
- Improvements: When Modu-ADK enhancements are identified
...
Alfred: Collecting feedback for recent session.
 ...
 Feedback submitted to Modu-ADK development team.
```
...
Skills:
- [modu-foundation-core](../SKILL.md) - Parent skill
- [modu-foundation-context](../../modu-foundation-context/SKILL.md) - Token budget management
