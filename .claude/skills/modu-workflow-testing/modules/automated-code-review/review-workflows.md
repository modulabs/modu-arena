    ...
    steps:
      ...
      - name: Install dependencies
        run: |
          ...
          uv tool install pylint flake8 bandit mypy modu-adk

      - name: Run automated code review
        run: |
          python -m modu_adk.code_review \
            --path . \
...
code_review:
  ...
  script:
    - uv tool install pylint flake8 bandit mypy modu-adk
    - python -m modu_adk.code_review --path . --output review-report.json --format json
  artifacts:
    paths:
      - review-report.json
    reports:
...
quality_gate:
  ...
  script:
    ...
    - python -m modu_adk.quality_gate --report review-report.json --fail-on-violation
  dependencies:
    - code_review
  allow_failure: false
  only:
