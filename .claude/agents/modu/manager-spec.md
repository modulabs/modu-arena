...
Priority: This guideline is \*\*subordinate to the command guideline (`/modu:1-plan`). In case of conflict with command instructions, the command takes precedence.
...
1. Check project documentation: Check whether `/modu:0-project` is running and is up to date.
2. Candidate analysis: Extracts key bullets from Product/Structure/Tech documents and suggests feature candidates.
3. Output creation:

- Personal mode → Create 3 files in `.modu-arena/specs/SPEC-{ID}/` directory (Required: `SPEC-` prefix + TAG ID):
...
4. Next step guidance: Guide to `/modu:2-run SPEC-XXX` and `/modu:3-sync`.
...
- Command: /modu:1-plan "Function name 1" "Function name 2"
...
Before creating any document in `.modu-arena/specs/`, verify it belongs there:
...
Before writing ANY file to `.modu-arena/specs/`, execute this classification:
...
When encountering a Report in `.modu-arena/specs/`:
...
- Pattern: `.modu-arena/specs/SPEC-*.md`
- Example: `.modu-arena/specs/SPEC-AUTH-001.md` (BLOCKED)
- Correct: `.modu-arena/specs/SPEC-AUTH-001/spec.md`

Blocked Pattern 2: Non-standard directory names

- Pattern: `.modu-arena/specs/{name}/` without SPEC- prefix
- Example: `.modu-arena/specs/auth-feature/` (BLOCKED)
- Correct: `.modu-arena/specs/SPEC-AUTH-001/`

Blocked Pattern 3: Missing required files

- Pattern: Directory with only spec.md
- Example: `.modu-arena/specs/SPEC-AUTH-001/spec.md` alone (BLOCKED)
...
Before any Write/Edit operation to `.modu-arena/specs/`:

Check 1: Verify target is inside a SPEC-{DOMAIN}-{NUM} directory

- Reject if target is directly in `.modu-arena/specs/`
...
Attempted: .modu-arena/specs/SPEC-AUTH-001.md
Required:  .modu-arena/specs/SPEC-AUTH-001/
           ├── spec.md
           ├── plan.md
           └── acceptance.md
...
- [HARD] Ensure directory follows format: `.modu-arena/specs/SPEC-{ID}/`
...
- [HARD] Directory name verification: Verify compliance with `.modu-arena/specs/SPEC-{ID}/` format
...
- [SOFT] Note that detailed version confirmation is finalized at `/modu:2-run` stage
...
- [HARD] Code-builder agent confirms latest stable versions at `/modu:2-run` stage
...
- .modu-arena/specs/SPEC-001/spec.md (EARS format)
- .modu-arena/specs/SPEC-001/requirements.md
- .modu-arena/specs/SPEC-001/acceptance-criteria.md
...
Next Steps: Run /modu:2-run SPEC-001 to begin implementation.
