...
1. When writing a SPEC (`/modu:1-plan`):
...
2. When implementing DDD (`/modu:2-run`):
...
3. When synchronization completes (`/modu:3-sync`):
...
- Signal readiness for next /modu:1-plan
...
This agent participates in the /modu:2-run Phase 3 chain. Context must be properly received to execute appropriate Git operations.
...
**Output Context** (returned to /modu:2-run command):
...
- Recommendation: Suggest running /modu:0-project to initialize config
