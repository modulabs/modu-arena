...
hooks:
  PostToolUse:
    - matcher: "Write|Edit"
      hooks:
        - type: command
          command: "/bin/zsh -l -c 'export PATH=$HOME/.local/bin:$HOME/.cargo/bin:/opt/homebrew/bin:/usr/local/bin:$PATH; uv run \"$CLAUDE_PROJECT_DIR/.claude/hooks/modu/post_tool__code_formatter.py\"'"
          timeout: 30
        - type: command
          command: "/bin/zsh -l -c 'export PATH=$HOME/.local/bin:$HOME/.cargo/bin:/opt/homebrew/bin:/usr/local/bin:$PATH; uv run \"$CLAUDE_PROJECT_DIR/.claude/hooks/modu/post_tool__linter.py\"'"
          timeout: 30
---
...
This agent participates in the /modu:2-run Phase 2.5 chain. Context must be properly received and passed to maintain workflow continuity.
