...
THEN API keys have prefix "modu_rank_"
...
**Feature**: LLM evaluates projects and stores passing results

**Scenario 1: Successful project evaluation**
```gherkin
GIVEN a user runs /modu-submit on a working project
WHEN the LLM evaluation completes with score >= 5
THEN the evaluation is stored in project_evaluations
...
WHEN the LLM evaluation completes with score < 5
THEN the evaluation is NOT stored
...
THEN the existing evaluation is returned
...
WHEN total_score is outside 0-10 range
...
AND includes functionality score (0-5)
AND includes practicality score (0-5)
AND includes total score (0-10)
