...
- [ ] Replace `moai` with `modu` in all TypeScript files
- [ ] Update API key prefix from `moai_rank_` to `modu_rank_`
...
**Success Criteria**: No remaining references to "moai", "MoAI", or "MOAI" in user-facing code
...
**Success Criteria**: Users can run `/modu-submit`, passing projects are stored and counted
